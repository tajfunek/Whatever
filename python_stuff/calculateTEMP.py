...
def convert(x, y, deg):
    ...
    #if x > 0: x += 1
    #if y < 0: y -= 1
    #if resX % 2 == 0: x += 0.5 #może być przydatne...
    return x, y, deg
...
def extract(filename, folder, stepDEGR=1):
    ...
    #PyPNG:
    reader = png.Reader(folder + filename)
    ...
    try:
        ...
        print("invalid filename - no stepDEGR")
        deg = 0
        cam_no = 1
    ...
    for i in range(len(pixels)):
        if (i % 3) == 0:
            pic.append(pixels[i+1]) #zmienić na i+1 - zielone punkty !
